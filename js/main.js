...
// ===================== HERO BIKES GRID (index.html only) =====================
function renderHeroGrid() {
  ...
  grid.innerHTML = bikes.map((bike, i) => {
    ...
    return `
      <a href="bike.html?id=${bike.id}" class="hbc-card" style="animation-delay:${delay}s">
        <div class="hbc-img">
        ...
        <div class="hbc-info">
          <div class="hbc-row">
            ...
            <span class="hbc-price">&#8369;${bike.pricePerDay.toLocaleString()}<em>/day</em></span>
          </div>
...
function buildBikeCard(bike) {
  ...
  return `
    <div class="bike-card">
      <a href="bike.html?id=${bike.id}" class="bike-card-img">${imgEl}</a>
      <div class="bike-card-body">
        ...
        <div class="bike-card-footer">
          <div class="bike-price">
            &#8369;${bike.pricePerDay.toLocaleString()}<span>/day</span>
          </div>
          <a href="bike.html?id=${bike.id}" class="btn-details">See Details</a>
        </div>
      </div>
    </div>`;
}
...
function renderFleet() {
  ...
  const sportbikes = bikes.filter(b => b.type === "sportbike");
  const nakedbikes = bikes.filter(b => b.type === "naked");
  const available  = bikes.filter(b => b.status === "available").length;

  sportGrid.innerHTML  = sportbikes.map(buildBikeCard).join("");
  nakedGrid.innerHTML  = nakedbikes.map(buildBikeCard).join("");
...
function renderDetail() {
  ...
  const availBlock = bike.status === "available"
    ? `<div class="detail-availability available">
        <div class="availability-status available">&#10003; Available for Rent</div>
       </div>`
  ...
  const imgEl = `<img src="${bike.image}" alt="${bike.name}" onerror="this.src='';this.parentElement.style.display='flex';this.parentElement.style.alignItems='center';this.parentElement.style.justifyContent='center';this.parentElement.innerHTML='<span style=\\'font-size:80px\\'>🏍️</span>'">`;
  ...
  root.innerHTML = `
    ...
    <div class="detail-hero">
      <div class="container">
        <div class="detail-hero-img">${imgEl}</div>
      </div>
    </div>
        ...
        <div class="detail-grid">
          ...
          <div class="detail-info">
            ...
            <div class="detail-specs">
              <div class="detail-spec">
                <div class="detail-spec-label">Engine</div>
                <div class="detail-spec-value">${bike.specs.engine}</div>
              </div>
              <div class="detail-spec">
                <div class="detail-spec-label">Power</div>
                <div class="detail-spec-value">${bike.specs.power}</div>
              </div>
              <div class="detail-spec">
                <div class="detail-spec-label">Torque</div>
                <div class="detail-spec-value">${bike.specs.torque}</div>
              </div>
              <div class="detail-spec">
                <div class="detail-spec-label">Transmission</div>
                <div class="detail-spec-value">${bike.specs.transmission}</div>
              </div>
              <div class="detail-spec">
                <div class="detail-spec-label">Weight</div>
                <div class="detail-spec-value">${bike.specs.weight}</div>
              </div>
              <div class="detail-spec">
                <div class="detail-spec-label">Seat Height</div>
                <div class="detail-spec-value">${bike.specs.seatHeight}</div>
              </div>
              <div class="detail-spec">
                <div class="detail-spec-label">Color</div>
                <div class="detail-spec-value">${bike.color}</div>
              </div>
              <div class="detail-spec">
                <div class="detail-spec-label">Year</div>
                <div class="detail-spec-value">${bike.year}</div>
              </div>
              <div class="detail-spec">
                <div class="detail-spec-label">Mileage</div>
                <div class="detail-spec-value">${bike.mileage}</div>
              </div>
            </div>
          </div>
          ...
          <div class="detail-sidebar">
            <div class="detail-sidebar-card">
              ...
              <div class="detail-price">&#8369;${bike.pricePerDay.toLocaleString()}<span> / day</span></div>

              ${availBlock}

              <a href="${FACEBOOK_URL}" target="_blank" class="btn btn-primary detail-book-btn">
                Book This Bike on Facebook
              </a>
              ...
              <div class="detail-contact">
                <div class="detail-contact-item">
                  <span>&#128222;</span>
                  <span>Call / Text: <a href="tel:${PHONE.replace(/\s/g,'')}">${PHONE}</a></span>
                </div>
                <div class="detail-contact-item">
                  <span>&#128172;</span>
                  <span><a href="${FACEBOOK_URL}" target="_blank">Message on Facebook</a></span>
                </div>
              </div>
            </div>
          </div>
